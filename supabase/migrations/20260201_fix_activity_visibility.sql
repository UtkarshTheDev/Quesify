-- Migration: Fix activity visibility and solution triggers
-- Description: Fixes two issues:
-- 1. Updates RLS policy for user_activities to ensure everyone can view them
-- 2. Fixes log_user_activity trigger to correctly identify solution contributions vs personal notes

-- ============================================
-- 1. FIX USER ACTIVITIES VISIBILITY
-- ============================================

-- Drop existing restrictive policy if it exists (recreating is safer)
DROP POLICY IF EXISTS "Public user_activities are viewable by everyone" ON public.user_activities;

-- Re-create policy to ensure it's truly public
CREATE POLICY "Public user_activities are viewable by everyone" 
ON public.user_activities FOR SELECT 
USING (true);

-- ============================================
-- 2. FIX ACTIVITY LOGGING TRIGGER
-- ============================================
...
BEGIN
    ...
    ELSIF TG_TABLE_NAME = 'solutions' THEN
        IF TG_OP = 'INSERT' THEN
            ...
            -- Logic: A "contribution" is when you add a solution to someone ELSE'S question.
            -- If you add a solution to a question you LINKED (but don't own), it's still a contribution to the platform.
            -- However, if you are just adding a solution to a question you OWN, it's part of creation (optional to log, but usually redundant if done immediately).
            
            -- FIX: We want to log it as 'solution_contributed' ONLY if the solution contributor is NOT the question owner.
            -- This correctly separates "I solved my own question" (rare/maintenance) from "I helped someone else" (contribution).
